# === script/grabber.gd (DROP-IN) ===
# Node2D with children: Sprite2D, GrabBox:Area2D(+CollisionShape2D), optional Hold:Timer, Think:Timer

extends Node2D

@export var hover_y: float = -120.0
@export var grab_y:  float =   40.0
@export var min_x:  float = -450.0
@export var max_x:  float =  450.0
@export var base_hover_speed: float = 220.0
@export var base_drop_speed:  float = 700.0
@export var wait_between_attacks: Vector2 = Vector2(1.0, 2.0)
@export var hold_time: float = 5.0

@export var hover_tex: Texture2D
@export var drop_tex:  Texture2D

# adaptive speed (treat "grabbed" as a HIT for slowing; MISS if it reaches bottom without grab)
@export var mult_init: float = 1.0
@export var mult_min:  float = 0.6
@export var mult_max:  float = 3.0
@export var miss_step: float = 0.08
@export var hit_step:  float = 0.10

enum { HOVER, DROPPING, HOLDING, RETURNING }
var _state: int = HOVER
var _target_x: float = 0.0
var _mult: float = 1.0
var _got_this_drop: bool = false

@onready var sprite:  Sprite2D = $Sprite2D
@onready var grabbox: Area2D   = get_node_or_null("GrabBox")
@onready var hold:    Timer    = get_node_or_null("Hold")
...
func _ready() -> void:
	_mult = mult_init
	if grabbox:
		grabbox.collision_layer = 0
		grabbox.collision_mask  = 1
		grabbox.monitoring = false
		if not grabbox.body_entered.is_connected(_on_body): grabbox.body_entered.connect(_on_body)
	if hold == null:
		hold = Timer.new(); hold.name = "Hold"; hold.one_shot = true; add_child(hold)
	if think == null:
		think = Timer.new(); think.name = "Think"; think.one_shot = true; add_child(think)
	if not hold.timeout.is_connected(_release): hold.timeout.connect(_release)
...
func _physics_process(delta: float) -> void:
	...
	match _state:
		...
		DROPPING:
			global_position.y = move_toward(global_position.y, grab_y, ds * delta)
			if is_equal_approx(global_position.y, grab_y):
				# adjust on result
				if _got_this_drop: _on_hit_adjust() 
				else: _on_miss_adjust()
				if _got_this_drop:
					_state = HOLDING
					hold.start(hold_time)
				else:
					_state = RETURNING
				if grabbox: grabbox.set_deferred("monitoring", false)
...
func _decide() -> void:
	if _state == HOVER and absf(global_position.x - _target_x) <= 0.5:
		_state = DROPPING
		_set_drop_tex()
		_got_this_drop = false
		if grabbox: grabbox.set_deferred("monitoring", true)
	_arm_think()
...
func _on_body(body: Node) -> void:
	...
	# Normal grab
	if _got_this_drop == false and body.has_method("on_grabbed"):
		_got_this_drop = true
		sprite.visible = false
		body.on_grabbed()

func _release() -> void:
	if _got_this_drop and body_exists(_got_this_drop):
		pass # player handles release below
	if _got_this_drop:
		_got_this_drop = false
...
# utils
func body_exists(val) -> bool: return true # placeholder (kept for compatibility)
